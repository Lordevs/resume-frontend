      ...
      fetchResume()
        .then((data) => {
          const normalised: Resume = {
            ...
            phone: data.phone || "",
            email: data.email || "",
            social_links: (data.social_links as SocialLink[] | undefined) || [], // NEW
  ...
  const addSocialLink = () => {
    if (!resume) return;
    if (resume.social_links.length >= 2) {
      alert("You can add up to 2 links (e.g. LinkedIn + GitHub).");
      return;
    }
    const newLink: SocialLink = { name: "", url: "" };
    setResume({ ...resume, social_links: [...resume.social_links, newLink] });
  };

  const removeSocialLink = (index: number) => {
    if (!resume) return;
    const links = resume.social_links.filter((_, i) => i !== index);
    setResume({ ...resume, social_links: links });
  };
          ...
          >
            ...
            <div style={sectionShell}>
              ...
              <p style={{ fontSize: 12, color: "#666", marginTop: -8 }}>
                Add up to 2 links (e.g. LinkedIn, GitHub, Portfolio).
              </p>
